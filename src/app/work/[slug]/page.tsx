...
import { experiences, getAdjacent, getExperience } from '@/data/experience';
  ...
  return pageMetadata({
    ...
    images: cover
      ? [
          {
            url: cover.src,
            alt: `${exp.company} website screenshot — ${cover.title}`,
            width: 1200,
            height: 750,
          },
        ]
    ...
    <article>
      ...
      <div className='site-gutter mx-auto grid max-w-6xl gap-10 py-10 sm:gap-14 sm:py-14 lg:grid-cols-[1fr_280px]'>
        ...
        <aside className='h-fit rounded-xl border border-line bg-canvas-deep/80 p-5 lg:sticky lg:top-24'>
          ...
          <p className='mt-6 text-xs leading-relaxed text-ink-faint'>
            Gallery images are historical website screenshots from Tim&apos;s
            time at {exp.shortName} ({exp.start}–{exp.end}).
          </p>
        </aside>
      </div>
      ...
      <section className='border-t border-line bg-canvas-deep/40'>
        <div className='site-gutter mx-auto max-w-6xl py-12 sm:py-16'>
          ...
          <p className='mt-3 max-w-2xl text-ink-muted'>
            Marketing and player-facing pages as they looked while Tim worked on
            the web platforms behind them.
          </p>
      ...
      <nav className='site-gutter mx-auto flex max-w-6xl justify-between gap-4 py-10 sm:py-12'>
        {prev ?
          <Link
            href={`/work/${prev.slug}`}
            className='group min-w-0 max-w-[48%]'
          >
        ...
        {next ?
          <Link
            href={`/work/${next.slug}`}
            className='group min-w-0 max-w-[48%] text-right'
          >
            <span className='text-xs uppercase tracking-wider text-ink-faint'>
              Next
            </span>
